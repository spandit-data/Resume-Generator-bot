...
A Telegram bot that helps blue collar workers in India create professional resumes through voice-based conversations in Hindi. The bot collects user responses via voice notes or text, transcribes Hindi audio using Groq Whisper, structures data with Google Gemini, and generates a professional PDF resume.
...
- `GROQ_API_KEY` - for Hindi speech-to-text (Whisper)
- `GEMINI_API_KEY` - for structuring raw answers into resume JSON
...
src/resume_builder.py   # Data structuring: raw answers → JSON (Gemini, different prompts for fresher/experienced)
...
6. After last answer, Gemini structures data into JSON (different prompts for fresher vs experienced)
...
- **Async/sync bridge** in bot.py: Gemini calls are synchronous, run in executor
...
docker run -e TELEGRAM_BOT_TOKEN=xxx -e GROQ_API_KEY=xxx -e GEMINI_API_KEY=xxx resume-bot
```
